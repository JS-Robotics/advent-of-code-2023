    ...
    for (char c : string_set.at(1)) {
      if (c != '.' && !CharIsNumber(c)) {
        if (c == '*') {
          ...
          if (sum_part != 0) {
            matches++;
            sum_ratio.emplace_back(sum_part);
          ...
          if (matches == 2) {
            std::cout << "Matched with two: " << c << " - ratio: " << sum_ratio.at(0) << " : "
                      << sum_ratio.at(sum_ratio.size() - 1) << " Size: " << sum_ratio.size() << std::endl;
            total_ratio += sum_ratio.at(0) * sum_ratio.at(1);
          }
        }
